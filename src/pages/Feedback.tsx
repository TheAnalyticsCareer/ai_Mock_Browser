...
const Feedback = () => {
  ...
  // Custom AI Score mapping based on transcript length
  function mapTranscriptLengthToScore(length: number): number {
  ...
  useEffect(() => {
    const arr = getTranscriptArray();
    setAiScore(mapTranscriptLengthToScore(arr.length));
    setManualScore(null); // Reset manual score if transcript changes
  ...
  const rating = manualScore !== null ? manualScore : aiScore;
            ...
            <div className="flex flex-col">
              ...
              <div className="mt-4 sm:mt-0 sm:text-right">
                <div className="bg-white bg-opacity-20 rounded-lg p-2 sm:p-3 inline-block">
                  ...
                  <div className="mt-2">
                    <label className="block text-xs text-blue-100 mb-1"></label>
                    <Slider
                      min={0}
                      max={8}
                      step={1}
                      value={[manualScore !== null ? manualScore : aiScore]}
                      onValueChange={([val]) => setManualScore(val)}
